...
{
    protected $baseSettingsLocation = 'uploads/academy/';
            ...
            foreach ($settings as $setting) {
                $this->tag->setDefault('meta['. $setting->key . ']', $setting->value);
                 ...
                 if($setting->key == 'academy_image')
                     $this->view->setVar("academyImage", $this->baseSettingsLocation . $setting->value);
            }
        ...
        if ($this->request->hasFiles() == true) {
            ...
            if(isset($_FILES['academy_image'])){
                foreach ($this->request->getUploadedFiles() as $file){
                    ...
                    $file->moveTo($this->baseSettingsLocation . $file->getName());
                    $path = pathinfo($this->baseSettingsLocation . $file->getName());
                    
                    $filename = $this->baseSettingsLocation . 'school' . '.' . $path['extension'];
                    rename($this->baseSettingsLocation . $path['basename'], $filename);
